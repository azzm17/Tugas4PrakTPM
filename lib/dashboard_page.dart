...
import 'package:tugas2navigation/detail_page.dart';
...
Widget _createListCard() {
  var dataDatepicker = getData;

  return ListView.separated(
    itemBuilder: (context, index) {
      return _createCard(dataDatepicker[index], context);
    },
    itemCount: dataDatepicker.length,
...
Widget _createCard(Datepicker data, BuildContext context) {
  return Card(
    child: InkWell(
      onTap: (){
        Navigator.push(context, MaterialPageRoute(builder: (context) {
          return DetailPage(title: data.title, name: data.name, price: data.price,);
        }));
      },
      child: Container(
        child: Row(
